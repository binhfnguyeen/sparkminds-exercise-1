...
export const MemberTable = ({
    ...
    const renderStatusBadge = (status: string) => {
        switch (status) {
            case 'ACTIVE':
                return <span className="inline-flex px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-100 text-emerald-700 border border-emerald-200">Đang HĐ</span>;
            case 'BLOCKED':
                return <span className="inline-flex px-2.5 py-1 rounded-full text-xs font-bold bg-rose-100 text-rose-700 border border-rose-200">Bị Khóa</span>;
            case 'UNVERIFIED':
                return <span className="inline-flex px-2.5 py-1 rounded-full text-xs font-bold bg-amber-100 text-amber-700 border border-amber-200">Chưa XN</span>;
            case 'DELETED':
                return <span className="inline-flex px-2.5 py-1 rounded-full text-xs font-bold bg-gray-100 text-gray-600 border border-gray-200">Đã Xóa</span>;
            default:
                return <span className="inline-flex px-2.5 py-1 rounded-full text-xs font-bold bg-gray-100 text-gray-600 border border-gray-200">{status}</span>;
        }
    };
                ...
                <table className="w-full text-left border-collapse whitespace-nowrap">
                    ...
                    {loading ? (
                        ...
                        <tr>
                            <td colSpan={5} className="p-12 text-center text-gray-500 font-medium">
                                <div className="flex flex-col items-center justify-center gap-3">
                                    <svg className="w-12 h-12 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path></svg>
                            ...
                            <tr key={member.id} className="hover:bg-blue-50/30 transition-colors group">
                                ...
                                <td className="px-6 py-4">
                                    <div className="flex flex-col">
                                        ...
                                        <span className="text-xs text-gray-500 font-medium mt-0.5">SN: {formatDate(member.dateOfBirth)}</span>
                                    </div>
                                </td>

                                <td className="px-6 py-4 text-gray-600 font-medium">
                                    {member.phone || <span className="text-gray-400 italic">Chưa cập nhật</span>}
                                </td>
                                    ...
                                    <div className="flex items-center justify-center gap-2 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
                                        ...
                                        {member.status === 'BLOCKED' ? (
                                            <button
                                                ...
                                                className="p-2 text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors focus:outline-none"
